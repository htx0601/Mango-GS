import torch
from mango import MangoDeformNetwork, MangoNodeWarp
import os
...
model_dict = {
    'mango': MangoDeformNetwork,
    'mango_node': MangoNodeWarp,
}


class DeformModel:
    def __init__(self, deform_type='mango_node', T=4, is_blender=False, d_rot_as_res=True, **kwargs):
        self.T = T
        if deform_type not in model_dict:
            raise ValueError(
                f"Unsupported deform_type '{deform_type}'. "
                "The public release supports only 'mango_node' and 'mango'."
            )
    ...
    def set_time_window(self, fid0, max_fid):
        end = min(fid0 + self.T, max_fid+1)
        t = torch.arange(fid0, end, device='cuda')
        if t.shape[0] < self.T:
            pad = t[-1].repeat(self.T - t.shape[0])
    ...
    def train_setting(self, training_args):
        ...
        self.optimizer = torch.optim.AdamW(l, lr=0.0, eps=1e-10)
