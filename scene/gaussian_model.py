...
class GaussianModel:
    ...
    def _softsign(x: torch.Tensor) -> torch.Tensor:
        # Simple robust compression to (-1, 1).
        return x / (1.0 + x.abs())

    @property
    def get_knn_feature(self):
        s = self.get_scaling  # >0
        s_code = self._softsign(torch.log1p(s))  # [B,N,ds]
        r_code = self.get_rotation  # normalized to [-1,1]
        o_code = self._softsign(self._opacity)  # pre
        return torch.cat([s_code, r_code, o_code], dim=-1)
    ...
    def densify_and_prune(self, max_grad, min_opacity, extent, max_screen_size):
        training_num = self.xyz_gradient_accum.shape[0]
        grads = self.xyz_gradient_accum / self.denom
        grads[grads.isnan()] = 0.0

        if training_num < 500000:
            self.densify_and_clone(grads, max_grad, extent)
            self.densify_and_split(grads, max_grad, extent)

        densify_num = self.get_xyz.shape[0]
        ...
        if training_num > 5000:
            self.prune_points(prune_mask)
        prune_num = self.get_xyz.shape[0]
        print(training_num, densify_num, prune_num)
    ...
    def add_densification_stats(self, viewspace_point_tensor, update_filter):
        ...
        # else:
            ...
            self.xyz_gradient_accum[update_filter] += viewspace_point_tensor
