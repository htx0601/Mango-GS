...
def readNerfiesCameras(path, inter_valid=False):
    ...
    if name.startswith('vrig'):
        ...
        train_img = list(np.array(dataset_json['ids'])[[i for i in range(len(dataset_json['ids'])) if i % 4 == 2]])
        val_img = list(np.array(dataset_json['ids'])[[i for i in range(len(dataset_json['ids'])) if i % 4 == 0]])
        all_img = train_img + val_img
        ratio = 0.5
    else:  # for hypernerf
...
# ====================== NEW: DyCheck iPhone camera loader ======================

def readDycheckIphoneCameras(path, inter_valid=False):
    # scene.json: center / scale
    with open(os.path.join(path, "scene.json"), "r") as f:
        scene_json = json.load(f)
    scene_center = np.array(scene_json["center"], dtype=np.float32)
    scene_scale = scene_json["scale"]

    # dataset.json / metadata.json / extra.json after iPhone preprocessing.
    ...
    factor = extra_json.get("factor", 2)     # default 2, corresponding to rgb/2x
    ratio = 1.0 / factor                     # intrinsics scaling ratio

    all_ids = np.array(dataset_json["ids"])
    train_img = list(all_ids[[i for i in range(len(all_ids)) if i % 2 == 1]])
    val_img = list(all_ids[[i for i in range(len(all_ids)) if i % 2 == 0]])
    all_img = train_img + val_img
    train_num = len(train_img)

    # iPhone uses warp_id as time.
    all_time_ids = [meta_json[i]["warp_id"] for i in all_img]
    max_time = max(all_time_ids) if len(all_time_ids) > 0 else 1
    all_time = [t / max_time for t in all_time_ids]

    # Read camera/*.json and apply Nerfies-style centering/scaling.
    all_cam_params = []
    for im in all_img:
        cam = camera_nerfies_from_JSON(
            os.path.join(path, "camera", f"{im}.json"), ratio
        )
        cam["position"] = (cam["position"] - scene_center) * scene_scale
        all_cam_params.append(cam)

    # Image directory: rgb/{factor}x/{frame_name}.png
    img_root = os.path.join(path, "rgb", f"{factor}x")
    depth_root = os.path.join(path, "depth", f"{factor}x")   # NEW

    cam_infos = []
    for idx, (im, fid) in enumerate(zip(all_img, all_time)):
        image_path = os.path.join(img_root, f"{im}.png")
        image = np.array(Image.open(image_path))
        image = Image.fromarray(image.astype(np.uint8))
        image_name = Path(image_path).stem
        ...
        # NEW: depth GT
        depth = None
        depth_path = os.path.join(depth_root, f"{im}.npy")
        if os.path.exists(depth_path):
            depth_np = np.load(depth_path).astype(np.float32)  # [H,W] or [H,W,1]
            depth_np = depth_np * scene_scale
            if depth_np.shape[-1] == 1:
                depth = np.transpose(depth_np, (2, 0, 1))  # (H,W,1) -> (1,H,W)
    ...
    sys.stdout.write("\n")
    return cam_infos, train_num, scene_center, scene_scale



# ====================== NEW: DyCheck iPhone scene loader ======================

# ====================== NEW: iPhone DyCheck scene info (no COLMAP) ======================

def readDycheckIphoneInfo(path, eval, downsample_max_pts=100000):
    print("Reading DyCheck iPhone Info")

    cam_infos, train_num, scene_center, scene_scale = readDycheckIphoneCameras(path)
    ...
    # Return normalization metadata without modifying cameras.
    nerf_normalization = getNerfppNorm(train_cam_infos, apply=False)
    nerf_normalization["radius"] *= 3

    # Point cloud from preprocessed points.npy.
    ply_path = os.path.join(path, "points3d.ply")
    if not os.path.exists(ply_path):
        print("Generating point cloud from DyCheck iPhone points.npy ...")

        xyz = np.load(os.path.join(path, "points.npy")).astype(np.float32)
        # Keep this consistent with the Nerfies no-COLMAP branch.
        xyz = (xyz - scene_center) * scene_scale

        # Optional downsampling to avoid excessive points.
        n_pts = xyz.shape[0]
        if downsample_max_pts is not None and n_pts > downsample_max_pts:
            print(f"Downsampling point cloud from {n_pts} to {downsample_max_pts} points...")
            idx = np.random.choice(n_pts, downsample_max_pts, replace=False)
            xyz = xyz[idx]
            n_pts = xyz.shape[0]
...
def readCamerasFromNpy(path, npy_file, split, hold_id, num_images=None, video_indices=None,
                       intrinsics_mode="scaled", camera_id_mode="numeric"):
    cam_infos = []

    # Keep only camera folders with frame images; N3V also includes camXX.mp4 files.
    video_paths = sorted(
        p for p in glob(os.path.join(path, 'cam*'))
        if os.path.isdir(p) and os.path.isdir(os.path.join(p, 'images'))
    )
    video_entries = []
    for pose_idx, video_path in enumerate(video_paths):
        folder_name = os.path.basename(video_path)
        if not folder_name.startswith('cam') or not folder_name[3:].isdigit():
            raise ValueError(f"Unexpected N3V camera folder name: {folder_name}")
        video_entries.append((pose_idx, int(folder_name[3:]), video_path))
    ...
    n_cameras = poses.shape[0]
    if len(video_paths) != n_cameras:
        raise ValueError(
            f"Expected {n_cameras} camera folders from {npy_file}, found {len(video_paths)} under {path}"
        )
    if camera_id_mode == "numeric":
        video_by_key = {numeric_id: (pose_idx, video_path)
                        for pose_idx, numeric_id, video_path in video_entries}
    elif camera_id_mode == "position":
        video_by_key = {pose_idx: (pose_idx, video_path)
                        for pose_idx, numeric_id, video_path in video_entries}
    else:
        raise ValueError(f"Unknown N3V camera_id_mode {camera_id_mode!r}")
    poses = np.concatenate(
        [poses[..., 1:2], -poses[..., :1], poses[..., 2:4]], -1)
    bottoms = np.array([0, 0, 0, 1]).reshape(
        1, -1, 4).repeat(poses.shape[0], axis=0)
    ...
    if video_indices is not None:
        video_list = list(video_indices)
    else:
        i_test = np.array(hold_id)
        video_list = i_test if split != 'train' else sorted(set(np.arange(n_cameras)) - set(i_test))

    for i in video_list:
        if i not in video_by_key:
            raise ValueError(
                f"Requested N3V camera index {i}, but available camera keys are {sorted(video_by_key)}"
            )
        pose_idx, video_path = video_by_key[i]
        c2w = poses[pose_idx]

        # Get the camXX folder name, such as 'cam00'.
        ...
        for idx, original_image_name in enumerate(images_names):
            image_path = os.path.join(images_dir, original_image_name)
            ...
            # 1. Compose the new image_name.
            # original_image_name: '0001.png'
            # video_folder_name:   'cam00'
            # new image_name:      'cam00_0001.png'
            image_name = f"{video_folder_name}_{original_image_name}"

            frame_time = idx / max(n_frames - 1, 1)

            if intrinsics_mode == "scaled":
                focal_x = focal * (image.size[0] / W)
                focal_y = focal * (image.size[1] / H)
            elif intrinsics_mode == "direct":
                focal_x = focal
                focal_y = focal
            else:
                raise ValueError(f"Unknown N3V intrinsics_mode {intrinsics_mode!r}")
...
def _scgs_effective_train_indices(path, n_cameras, hold_id):
    raw_paths = sorted(glob(os.path.join(path, 'cam*')))
    hold = set(int(i) for i in hold_id)
    indices = []
    for i in sorted(set(range(n_cameras)) - hold):
        if i < len(raw_paths) and os.path.isdir(os.path.join(raw_paths[i], 'images')):
            name = os.path.basename(raw_paths[i])
            if name.startswith('cam') and name[3:].isdigit():
                indices.append(int(name[3:]))
    return sorted(set(indices))


def _camera_names(cam_infos):
    return {c.image_name.split("_")[0] for c in cam_infos}


def readPlenopticVideoDataset(path, eval, num_images=None, hold_id=[0], protocol="full_heldout",
                              intrinsics_mode="scaled", camera_id_mode="numeric"):
    poses_bounds = np.load(os.path.join(path, 'poses_bounds.npy'))
    n_cameras = poses_bounds.shape[0]
    if num_images is not None and num_images <= 0:
        num_images = None
    if protocol == "short24_clean" and num_images is None:
        num_images = 24

    hold = sorted(set(int(i) for i in hold_id))
    train_indices = sorted(set(range(n_cameras)) - set(hold))
    test_indices = hold
    if protocol == "scgs_subset_strict":
        train_indices = _scgs_effective_train_indices(path, n_cameras, hold)
        if num_images is None:
            num_images = 24
    elif protocol not in {"full_heldout", "short24_clean"}:
        raise ValueError(f"Unknown N3V protocol {protocol!r}")

    print(
        f"N3V protocol: {protocol}; num_images={num_images or 'all'}; "
        f"train_camera_indices={train_indices}; test_camera_indices={test_indices}; "
        f"intrinsics_mode={intrinsics_mode}; "
        f"camera_id_mode={camera_id_mode}"
    )

    print("Reading Training Camera")
    train_cam_infos = readCamerasFromNpy(path, 'poses_bounds.npy', split="train", hold_id=hold_id,
                                         num_images=num_images, video_indices=train_indices,
                                         intrinsics_mode=intrinsics_mode,
                                         camera_id_mode=camera_id_mode)

    print("Reading Test Camera")
    test_cam_infos = readCamerasFromNpy(
        path, 'poses_bounds.npy', split="test", hold_id=hold_id, num_images=num_images, video_indices=test_indices,
        intrinsics_mode=intrinsics_mode, camera_id_mode=camera_id_mode)

    shared_names = _camera_names(train_cam_infos) & _camera_names(test_cam_infos)
    if shared_names:
        raise ValueError(f"N3V camera split is invalid: {sorted(shared_names)}.")
