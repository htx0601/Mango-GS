...
def render_batch(frame_group, pc: GaussianModel, pipe, bg_color: torch.Tensor, d_xyz_stack, d_rotation_stack, d_scaling_stack,
           d_opacity_stack=None, d_color_stack=None, real_positions=None, scaling_modifier=1.0, **kwargs):

    renders, v_points, v_filters, radiis, depths, alphas, bgs = [], [], [], [], [], [], []
    iters = real_positions if real_positions is not None else [i for i in range(len(d_xyz_stack))]
    for i in iters:
        i = int(i)
        res = render(
            frame_group[i], pc, pipe, bg_color,
            d_xyz_stack[i], d_rotation_stack[i], d_scaling_stack[i],
            d_opacity=None if d_opacity_stack is None else d_opacity_stack[i],
            d_color=None if d_color_stack is None else d_color_stack[i],
            scaling_modifier=scaling_modifier,
            **kwargs
        )
        renders.append(res['render'])
        v_points.append(res['viewspace_points'])
        v_filters.append(res['visibility_filter'])
        radiis.append(res['radii'])
        depths.append(res['depth'])
        alphas.append(res['alpha'])
        bgs.append(res['bg_color'])

    return {
        "render": torch.stack(renders, dim=0),
            "viewspace_points": v_points,
            "visibility_filter": v_filters,
            "radii": torch.stack(radiis, dim=0),
            "depth": torch.stack(depths, dim=0),
            "alpha": torch.stack(alphas, dim=0),
            "bg_color": torch.stack(bgs, dim=0)
    }
